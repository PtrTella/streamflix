...
object CB01Provider : Provider {
    ...
    override suspend fun getMovie(id: String): Movie {
        ...
        return Movie(
            ...
            title = title,
            poster = poster,
            overview = tmdbMovie?.overview?.takeIf { it.isNotBlank() } ?: doc.select("div.ignore-css > p").firstOrNull { !it.text().contains("DURATA", true) }?.text()?.replace(Regex("\\s*\\+?Info\\s*»\\s*$", RegexOption.IGNORE_CASE), "")?.trim(),
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        return TvShow(
            ...
            overview = tmdbTvShow?.overview?.takeIf { it.isNotBlank() } ?: doc.select("div.ignore-css > p").firstOrNull { !it.text().contains("DURATA", true) }?.clone()?.apply { select("strong, b").remove() }?.text()?.replace(Regex("\\s*\\+?Info\\s*»\\s*$", RegexOption.IGNORE_CASE), "")?.trim(),
