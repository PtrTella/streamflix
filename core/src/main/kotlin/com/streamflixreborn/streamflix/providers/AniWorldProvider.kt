...
object AniWorldProvider : Provider {
    ...
    class TvShowDao {
        private val shows = java.util.concurrent.CopyOnWriteArrayList<TvShow>()
        fun searchTvShows(query: String, limit: Int, offset: Int): List<TvShow> =
            shows.filter { it.title.contains(query, ignoreCase = true) }.drop(offset).take(limit)
        fun getAll(): kotlinx.coroutines.flow.Flow<List<TvShow>> = kotlinx.coroutines.flow.flowOf(shows.toList())
        fun getAllIds(): List<String> = shows.map { it.id }
        fun insertAll(newShows: List<TvShow>) {
            val existing = shows.map { it.id }.toSet()
            newShows.filter { it.id !in existing }.forEach { shows.add(it) }
        }
    }
    ...
    private fun getDao(): TvShowDao = tvShowDao
